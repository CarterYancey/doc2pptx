...
6. Produce a detailed, slide-ready summary that captures the video's structure, main ideas, supporting details, examples, and conclusions.
...
- If the video has a chronological flow, reflect that in order.
- If the video is instructional, make the steps explicit.
    ...
    "json_schema": {
        ...
        "schema": {
            ...
            "properties": {
                ...
                "examples_or_case_studies": {
                    "type": "array",
                    "items": {"type": "string"}
                },
                "important_quotes_or_claims": {
                    "type": "array",
                    "items": {"type": "string"}
                },
                "terminology_and_entities": {
                    "type": "array",
                    "items": {"type": "string"}
                },
                "open_questions": {
                ...
                "slide_outline": {
                    ...
                    "items": {
                        ...
                        "required": ["slide_title", "purpose", "bullet_points"]
                    }
                }
            },
            "required": [
...
def to_markdown(summary: Dict[str, Any]) -> str:
    ...
    add_list_section("Timeline or Flow", summary["timeline_or_flow"])
    add_list_section("Examples or Case Studies", summary["examples_or_case_studies"])
    add_list_section("Important Quotes or Claims", summary["important_quotes_or_claims"])
    add_list_section("Terminology and Entities", summary["terminology_and_entities"])
    add_list_section("Open Questions", summary["open_questions"])

    lines.append("## Slide Outline\n")
    for i, slide in enumerate(summary["slide_outline"], 1):
        lines.append(f"### Slide {i}: {slide['slide_title']}")
...
def summarize_file(path: Path) -> None:
    ...
    try:
        ...
        json_path = OUTPUT_DIR / f"{path.stem}.summary.json"
        md_path = OUTPUT_DIR / f"{path.stem}.summary.md"
...
def main():
    txt_files = sorted(INPUT_DIR.glob("*1.txt"))
    if not txt_files:
